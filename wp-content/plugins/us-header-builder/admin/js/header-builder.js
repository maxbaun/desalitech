	...
	$usof.field['header_builder'] = {
		...
		_updateElementPlaceholder: function($elm, id, values){
			...
			if (type == 'text' && (values.text || values.icon)) {
				if (values.icon) {
					content += '<i class="' + $usof.instance.prepareIconClass(values.icon) + '"></i>';
				}
				// Strip tags
				...
				$.each(values, function(key, value){
					if (key == 'style' || key == 'hover' || key.substr(0, 7) == 'custom_' || key.substr(0, 4) == 'size' || key == 'design_options') return;
					if (value != '') socialsHtml += '<i class="fa fa-' + key + '"></i>';
				});
				if (values.custom_icon && values.custom_url) {
					socialsHtml += '<i class="' + $usof.instance.prepareIconClass(values.custom_icon) + '"></i>';
				}
