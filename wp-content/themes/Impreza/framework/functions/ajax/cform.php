...
function us_ajax_cform() {
	...
	if ( isset( $fields['message'] ) AND isset( $shortcode_atts['message_field'] ) AND $shortcode_atts['message_field'] != 'hidden' AND $_POST[ 'message' ] != '' ) {
		$body_message_part = ":</p><p>" . sanitize_text_field( stripslashes( $_POST[ 'message' ] ) ) . "</p>";
	} else {
		$body_message_part = '.</p>';
	}
	...
	if ( $body_phone_part != '' OR $body_email_part != '' ) {
		$body_contacts_part = ' (';
		if ( $body_phone_part != '' ) {
			$body_contacts_part .= $body_phone_part;
			if ( $body_email_part != '' ) {
				$body_contacts_part .= ', ';
			}
		}
		if ( $body_email_part != '' ) {
			$body_contacts_part .= $body_email_part;
		}
		$body_contacts_part .= ')';
	} else {
		$body_contacts_part = '';
	...
	$email_body = '<p>' . __( 'You received a message from', 'us' ) . ' <a href="' . site_url() . '">' . get_bloginfo( 'name' ) . "</a>.</p><p>" . $body_name_part . $body_contacts_part . ' ' . _x( 'wrote', 'smb wrote a message', 'us' ) . $body_message_part;
