...
class RWMB_Autocomplete_Field extends RWMB_Multiple_Values_Field
{
	...
	static function admin_enqueue_scripts()
	{
		...
		wp_localize_script( 'rwmb-autocomplete', 'RWMB_Autocomplete', array( 'delete' => us_translate_with_external_domain( 'Delete', 'meta-box' ) ) );
	}
			...
			foreach ( $field['options'] as $value => $label )
			{
				if ( in_array( $value, $meta ) )
				{
					$html .= sprintf(
						$tpl,
						$label,
						us_translate_with_external_domain( 'Delete', 'meta-box' ),
						$field['field_name'],
						$value
					);
				}
