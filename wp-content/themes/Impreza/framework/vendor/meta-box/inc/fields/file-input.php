...
class RWMB_File_Input_Field extends RWMB_Field
{
	...
	static function admin_enqueue_scripts()
	{
		...
		wp_localize_script( 'rwmb-file-input', 'rwmbFileInput', array(
			'frameTitle' => us_translate_with_external_domain( 'Select File', 'meta-box' ),
		) );
	}
	...
	static function html( $meta, $field )
	{
		return sprintf(
			...
			$field['placeholder'],
			$field['size'],
			us_translate_with_external_domain( 'Select', 'meta-box' ),
			$meta ? '' : 'hidden',
			us_translate_with_external_domain( 'Remove', 'meta-box' )
		);
	}
