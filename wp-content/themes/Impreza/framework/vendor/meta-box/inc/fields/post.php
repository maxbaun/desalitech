...
class RWMB_Post_Field extends RWMB_Object_Choice_Field
{
	...
	public static function normalize( $field )
	{
		...
		{
			$field['placeholder'] = us_translate_with_external_domain( 'Select a post', 'meta-box' );
			if ( is_string( $field['query_args']['post_type'] ) && post_type_exists( $field['query_args']['post_type'] ) )
			{
				...
				$field['placeholder'] = sprintf( us_translate_with_external_domain( 'Select a %s', 'meta-box' ), $post_type_object->labels->singular_name );
			}
		}
